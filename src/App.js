...
function App() {
  ...
  const refineMessage = () => {
    ...
    setMessages((prev) => [...prev, { sender: 'user', text: `Refine: ${input}` }]);
  };
  ...
  const resubmitIdea = () => {
    const lastIdea = messages.findLast((msg) => msg.sender === 'user')?.text || '';
    ...
    <div className="chat-box">
      ...
      <div className="messages">
        {messages.map((msg, idx) =>
          msg.sender === 'image' ? (
            <img key={idx} src={msg.url} alt="Generated" className="image-preview" />
          ) : (
